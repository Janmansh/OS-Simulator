...
void firstfit(vector <Block1*>& blocks, int largest_block, int unused){
...
int first_fit_delete(){
    ...
    for (size_t i = 0; i < n; i++)
    {
        ...
        if (block_size > remaining_size){
            ...
            blocks.push_back(new Block1(0));
        }
        else{
            if(block_size > largest_block)
                largest_block = block_size;
            remaining_size -= block_size;
            blocks.push_back(new Block1 (block_size));
            pos++;
            cout << "PA" << i << " is accomodated" << endl;
        }
    }
    firstfit(blocks, largest_block, remaining_size);
